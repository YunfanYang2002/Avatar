...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'emotion' in self.data[index]:
            emo = self.data[index]['emotion'] 
        else:
            emo = 'neutral'
        ...
        return torch.FloatTensor(audio),torch.FloatTensor(vertice), torch.FloatTensor(template), torch.FloatTensor(one_hot), file_name, emo
...
'''def read_data(
    ...
    #print('wav_path',wav_path,'vertices_path',vertices_path)
    ...
    audio_path = os.path.join(dataset_root, wav_path)
    vertices_path = os.path.join(dataset_root, vertices_path)
    wav2vec_path = os.path.join(dataset_root, "wav2vec2-base-960h")
    ...
    template_file = os.path.join(dataset_root, template_file)
