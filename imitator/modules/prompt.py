...
class EmotionPrompt(nn.Module):
    def __init__(self,latent_dim=16,style_dim=128):
        super(EmotionPrompt, self).__init__()
        self.mappingnet = MappingNetwork(latent_dim=latent_dim, style_dim=style_dim, num_domains=8, hidden_dim=512)
...
class EmotionDeepPrompt(nn.Module):
    def __init__(self,latent_dim=16,style_dim=128):
        ...
        self.mappingnet = MappingDeepNetwork(latent_dim=latent_dim, style_dim=style_dim, num_domains=8, hidden_dim=512)
        self.style_dim=128
    ...
    def forward(self,x):
        ...
        z_trg = x['z_trg']
        y_org = x['y_trg']
        print('z_trg.shape',z_trg.shape)
        print('y_org',y_org)
        bs = y_org.shape[0]
        s_trg = self.mappingnet(z_trg, y_org).reshape(bs, -1, 64)
        return s_trg[:,0,:], s_trg[:,1:,:]

from sync_batchnorm import SynchronizedBatchNorm2d as BatchNorm2d
from utilss.JDC.model import JDCNet
...
class EmotionalDeformationTransformer(nn.Module): # EDN
    def __init__(self, embedding_dim=256, num_kp=15, num_w=5):
